...
import type { PublicationRepository } from '@/modules/rust/domain/publication-repository'
import { rustPublicationMetadata } from '@/modules/rust/content/publication-metadata'
...
function readPublicationPayload(filePath: string, language: Language) {
  ...
  const metadata = rustPublicationMetadata[language][slug]
  ...
  const title = titleLine ? titleLine.slice(2).trim() : slugToTitle(slug)
  ...
  const description = firstParagraphLines.join(' ')
...
function getContentDirectory(language: Language) {
  return path.join(process.cwd(), 'src/modules/rust/content', language)
}
