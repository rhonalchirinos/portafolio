...
import type { PublicationRepository } from '@/modules/docker/domain/publication-repository'
...
function readPublicationPayload(filePath: string, language: Language) {
  ...
  const metadata = dockerPublicationMetadata[language][slug]
...
function getContentDirectory(language: Language) {
  return path.join(process.cwd(), 'src/modules/docker/content', language)
}
